...
VulkanBufferDispatchResult run_vulkan_swiglu_buffers(
    VulkanComputeContext& context,
  ...
  if (!gate.valid() || !up.valid() || !output.valid()) {
    result.diagnostic = "SwiGLU requires valid Vulkan buffers.";
    return result;
  }
  if (gate.size() == 0U ||
      gate.size() != up.size() ||
      gate.size() != output.size()) {
    result.diagnostic =
        "SwiGLU Vulkan buffer sizes must be equal and non-zero.";
    return result;
  }
  if (gate.size() > std::numeric_limits<std::uint32_t>::max()) {
    result.diagnostic = "SwiGLU input exceeds Vulkan bootstrap limits.";
  ...
  try {
    ...
    if (gate.native_device() != context.native_device() ||
        up.native_device() != context.native_device() ||
        output.native_device() != context.native_device()) {
      result.diagnostic =
          "SwiGLU Vulkan buffers belong to a different device.";
      return result;
    }

    const auto gate_buffer =
        reinterpret_cast<VkBuffer>(gate.native_buffer());
    const auto up_buffer =
        reinterpret_cast<VkBuffer>(up.native_buffer());
    const auto output_buffer =
        reinterpret_cast<VkBuffer>(output.native_buffer());

    if (gate_buffer == VK_NULL_HANDLE ||
        up_buffer == VK_NULL_HANDLE ||
        output_buffer == VK_NULL_HANDLE) {
      result.diagnostic = "SwiGLU Vulkan buffer handle is null.";
      return result;
    }
    ...
    VkShaderModuleCreateInfo shader_ci{
        ...
        .flags = 0,
        .codeSize =
            generated::kSwiGluSpirv.size() * sizeof(std::uint32_t),
        .pCode = generated::kSwiGluSpirv.data(),
    };
    ...
    const VkDescriptorBufferInfo infos[3]{
        {.buffer = gate_buffer, .offset = 0,
         .range = static_cast<VkDeviceSize>(gate.size_bytes())},
        {.buffer = up_buffer, .offset = 0,
         .range = static_cast<VkDeviceSize>(up.size_bytes())},
        {.buffer = output_buffer, .offset = 0,
         .range = static_cast<VkDeviceSize>(output.size_bytes())},
    };
    ...
    result.executed = true;
    result.diagnostic =
        "Vulkan SwiGLU completed between persistent activation buffers.";
    return result;
  } catch (const std::exception& e) {
    result.diagnostic =
        std::string("Vulkan SwiGLU buffer exception: ") + e.what();
    return result;
  } catch (...) {
    result.diagnostic =
        "Vulkan SwiGLU buffer dispatch encountered an unknown exception.";
    return result;
  }
}
