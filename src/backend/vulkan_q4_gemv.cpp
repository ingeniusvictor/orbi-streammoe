...
VulkanBufferDispatchResult run_vulkan_q4_gemv_preloaded(
    VulkanComputeContext& context,
  ...
  if (!context.valid() || !weights.valid() || !x.valid() || !y.valid()) {
    result.diagnostic =
        "Preloaded Q4 GEMV requires valid context, weights, and buffers.";
    return result;
  }

  if (x.size() != weights.in_dim() || y.size() != weights.out_dim()) {
    result.diagnostic =
        "Preloaded Q4 GEMV activation shapes do not match projection.";
    return result;
  }

  if (weights.native_device() != context.native_device() ||
      x.native_device() != context.native_device() ||
      y.native_device() != context.native_device() ||
      weights.scales_buffer().native_device() != context.native_device() ||
      weights.biases_buffer().native_device() != context.native_device()) {
    result.diagnostic =
        "Preloaded Q4 GEMV resources belong to a different Vulkan device.";
    return result;
  }

  if (weights.out_dim() > std::numeric_limits<std::uint32_t>::max() ||
      weights.packed_cols() > std::numeric_limits<std::uint32_t>::max() ||
      weights.in_dim() > std::numeric_limits<std::uint32_t>::max() ||
      weights.group_size() > std::numeric_limits<std::uint32_t>::max()) {
    result.diagnostic =
        "Preloaded Q4 GEMV shape exceeds Vulkan bootstrap limits.";
    return result;
  }
  ...
  try {
    ...
    const auto queue =
        reinterpret_cast<VkQueue>(context.native_queue());
    const auto command_pool =
        reinterpret_cast<VkCommandPool>(context.native_command_pool());

    const auto packed_buffer =
        reinterpret_cast<VkBuffer>(weights.native_packed_buffer());
    const auto scale_buffer = reinterpret_cast<VkBuffer>(
        weights.scales_buffer().native_buffer());
    const auto bias_buffer = reinterpret_cast<VkBuffer>(
        weights.biases_buffer().native_buffer());
    const auto input_buffer =
        reinterpret_cast<VkBuffer>(x.native_buffer());
    const auto output_buffer =
        reinterpret_cast<VkBuffer>(y.native_buffer());
    ...
    if (device == VK_NULL_HANDLE ||
        queue == VK_NULL_HANDLE ||
        command_pool == VK_NULL_HANDLE ||
        packed_buffer == VK_NULL_HANDLE ||
        scale_buffer == VK_NULL_HANDLE ||
        bias_buffer == VK_NULL_HANDLE ||
        input_buffer == VK_NULL_HANDLE ||
        output_buffer == VK_NULL_HANDLE) {
      result.diagnostic =
          "Preloaded Q4 GEMV encountered a null Vulkan handle.";
      return result;
    }

    constexpr std::uint32_t kLocalSize = 64U;
    const auto out_dim =
        static_cast<std::uint32_t>(weights.out_dim());
    const std::uint32_t workgroups =
        (out_dim + kLocalSize - 1U) / kLocalSize;
    ...
    const VkDescriptorBufferInfo infos[5]{
        {
            ...
            .offset = 0,
            .range = static_cast<VkDeviceSize>(
                weights.packed_size_bytes()),
        },
        {
            .buffer = scale_buffer,
            .offset = 0,
    ...
    const Q4GemvPushConstants params{
        .out_dim = static_cast<std::uint32_t>(weights.out_dim()),
        .packed_cols =
            static_cast<std::uint32_t>(weights.packed_cols()),
        .in_dim = static_cast<std::uint32_t>(weights.in_dim()),
        .group_size =
            static_cast<std::uint32_t>(weights.group_size()),
    };
    ...
    result.executed = true;
    result.diagnostic =
        "Preloaded affine Q4 GEMV completed without weight re-upload.";
    return result;
  } catch (const std::exception& e) {
    result.diagnostic =
        std::string("Preloaded Q4 GEMV exception: ") + e.what();
    return result;
  } catch (...) {
    result.diagnostic =
        "Preloaded Q4 GEMV encountered an unknown exception.";
    return result;
  }
}
