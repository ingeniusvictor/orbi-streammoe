...
int main() {
  const auto root =
      fs::temp_directory_path() / "orbi-streammoe-osm23-weighted-routed-moe";
  ...
  try {
    ...
    if (!context.has_value()) {
      ...
      fs::remove_all(root);
      std::cout
          << "OSM-23 weighted routed MoE: PASS (no compute device on host)\n";
      return 0;
    }
    ...
    // Row-major router [3 experts, 128 hidden]. Only x[0] contributes:
    // E0=1, E1=3, E2=2 => Top-2 is E1 then E2.
    ...
    for (const auto& pick : route.picks) {
      const auto& expert_values =
          pick.expert == 1U ? e1 : e2;
      for (std::size_t i = 0; i < expected.size(); ++i) {
        expected[i] += pick.weight * expert_values[i];
      }
    }

    const auto first = run_weighted_routed_moe(
        gpu_cache,
        0,
        x,
        router_weight,
    ...
    require(
        first_error <= 0.02F,
        "first weighted routed MoE CPU parity failed");
    ...
    const auto second = run_weighted_routed_moe(
        gpu_cache,
        0,
        x,
        router_weight,
    ...
    require(
        second_error <= 0.02F,
        "repeat weighted routed MoE CPU parity failed");
    ...
    fs::remove_all(root);
    std::cout
        << "OSM-23 weighted routed MoE: PASS\n"
        << "  device=" << context->info().device_name << "\n"
        << "  top1=expert_" << first.routing.picks[0].expert
        << " weight=" << first.routing.picks[0].weight << "\n"
        << "  top2=expert_" << first.routing.picks[1].expert
        << " weight=" << first.routing.picks[1].weight << "\n"
        ...
        << "  unselected_expert_not_loaded=PASS\n"
        << "  repeat_bypasses_host_cache=PASS\n";
    return 0;
  } catch (const std::exception& e) {
    fs::remove_all(root);
    std::cerr
        << "OSM-23 weighted routed MoE: FAIL: "
        << e.what() << "\n";
    return 1;
  }
}
