...
fs::path make_container(const fs::path& root) {
  ...
  const std::string manifest =
      ...
      "  \"sourceCheckpoint\": \"windows-overlapped-fixture\",\n"
      "  \"quantBits\": 4,\n"
      "  \"quantGroupSize\": 8,\n"
      "  \"files\": {\n"
...
void check_bytes(
  ...
  for (std::size_t i = 0; i < bytes.size(); ++i) {
    ...
    require(actual == expected, "overlapped expert bytes mismatch");
  }
}

void test_batched_overlapped_reads(const fs::path& root) {
  WindowsOverlappedExpertStorage storage(make_container(root));
...
void test_cache_integration(const fs::path& root) {
  WindowsOverlappedExpertStorage storage(root);
...
void test_short_read_fails(const fs::path& root) {
  ...
  require(failed, "short overlapped read was accepted");
}
...
int main() {
  const auto root =
      fs::temp_directory_path() / "orbi-streammoe-osm05-windows-overlapped";

  try {
    test_batched_overlapped_reads(root);
    test_cache_integration(root);
    test_short_read_fails(root);
    fs::remove_all(root);

    std::cout << "OSM-05 Windows overlapped expert I/O: PASS\n";
    return 0;
  } catch (const std::exception& e) {
    fs::remove_all(root);
    std::cerr
        << "OSM-05 Windows overlapped expert I/O: FAIL: "
        << e.what()
        << "\n";
    return 1;
  }
