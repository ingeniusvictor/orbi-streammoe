...
int main() {
  const auto root =
      fs::temp_directory_path() / "orbi-streammoe-osm21-gpu-cache";

  try {
    QpackExpertStorage storage(make_qpack(root));
    const auto oracle0 = make_oracle(storage, 0);
    ...
    if (!context.has_value()) {
      ...
      fs::remove_all(root);
      std::cout
          << "OSM-21 Vulkan resident expert cache: PASS (no compute device on host)\n";
      return 0;
    }
    ...
    auto* expert0 = gpu_cache.get(0, 0, &diagnostic);
    require(expert0 != nullptr, diagnostic);
    require(
        expert0->accounted_bytes() == kAccountedExpertBytes,
        "unexpected expert byte accounting");

    auto* expert1 = gpu_cache.get(0, 1, &diagnostic);
    require(expert1 != nullptr, diagnostic);

    auto stats = gpu_cache.stats();
    require(stats.hits == 0U, "initial cache hits mismatch");
    require(stats.misses == 2U, "initial cache misses mismatch");
    require(stats.loads == 2U, "initial cache loads mismatch");
    require(stats.evictions == 0U, "unexpected initial eviction");
    require(stats.resident_entries == 2U, "expected two GPU residents");
    require(
        stats.resident_bytes == 2U * kAccountedExpertBytes,
        "GPU resident byte budget mismatch");

    const auto host_before_hit = host_cache.stats();
    expert0 = gpu_cache.get(0, 0, &diagnostic);
    require(expert0 != nullptr, diagnostic);
    const auto host_after_hit = host_cache.stats();

    require(
        host_after_hit.hits == host_before_hit.hits &&
        host_after_hit.misses == host_before_hit.misses,
        "GPU cache hit must bypass host cache");

    auto* expert2 = gpu_cache.get(0, 2, &diagnostic);
    require(expert2 != nullptr, diagnostic);

    stats = gpu_cache.stats();
    require(stats.hits == 1U, "GPU hit count mismatch");
    require(stats.misses == 3U, "GPU miss count mismatch");
    require(stats.loads == 3U, "GPU load count mismatch");
    require(stats.evictions == 1U, "GPU eviction count mismatch");
    require(stats.resident_entries == 2U, "GPU cache size mismatch");
    require(
        stats.resident_bytes <= stats.budget_bytes,
        "GPU cache exceeded configured logical budget");

    require(gpu_cache.contains(0, 0), "frequent expert 0 should survive");
    require(!gpu_cache.contains(0, 1), "LFU expert 1 should be evicted");
    require(gpu_cache.contains(0, 2), "expert 2 should be resident");

    const auto x0 = token(0);
    const auto expected0 = run_oracle(oracle0, x0);
    const auto actual0 = gpu_cache.run(0, 0, x0);
    require(actual0.executed, actual0.diagnostic);
    require(
        max_abs_error(actual0.values, expected0) <= 0.01F,
        "retained expert 0 CPU parity failed");

    const auto x2 = token(2);
    const auto expected2 = run_oracle(oracle2, x2);
    const auto actual2 = gpu_cache.run(0, 2, x2);
    require(actual2.executed, actual2.diagnostic);
    require(
        max_abs_error(actual2.values, expected2) <= 0.01F,
        "new expert 2 CPU parity failed");

    stats = gpu_cache.stats();
    require(stats.hits == 3U, "final GPU hit count mismatch");
    require(stats.misses == 3U, "final GPU miss count mismatch");
    require(stats.loads == 3U, "final GPU load count mismatch");
    require(stats.evictions == 1U, "final GPU eviction count mismatch");

    fs::remove_all(root);
    std::cout
        << "OSM-21 Vulkan resident expert cache: PASS\n"
        ...
        << "  lfu_victim=expert_1 PASS\n"
        << "  gpu_hit_bypasses_host_cache=PASS\n";
    return 0;
  } catch (const std::exception& e) {
    fs::remove_all(root);
    std::cerr
        << "OSM-21 Vulkan resident expert cache: FAIL: "
        << e.what() << "\n";
    return 1;
  }
}
