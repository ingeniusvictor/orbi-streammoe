...
int main() {
  const auto root =
      fs::temp_directory_path() / "orbi-streammoe-osm24a-resident-output";

  try {
    QpackExpertStorage storage(make_qpack(root));
    ExpertCache cache(storage, storage.expert_stride_bytes(), 1U);

    const std::vector<std::uint32_t> pick{1};
    const auto entries = cache.fetch(0, pick);
    require(entries.size() == 1U, "expected one source expert");
    ...
    if (!context.has_value()) {
      ...
      fs::remove_all(root);
      std::cout
          << "OSM-24A resident Vulkan output: PASS (no compute device on host)\n";
      return 0;
    }
    ...
    auto shared_input =
        VulkanFloatBuffer::create(*context, resident->input_dim(), &diagnostic);
    require(shared_input.has_value(), diagnostic);
    require(shared_input->valid(), "shared input Vulkan buffer must be valid");

    const auto* output_before = resident->output_buffer();
    require(output_before != nullptr, "resident output buffer must exist");
    const auto output_handle = output_before->native_buffer();
    require(output_handle != 0U, "resident output handle must be non-zero");

    float worst_error = 0.0F;

    for (std::size_t token_index = 0; token_index < 2; ++token_index) {
      std::vector<float> x(128);
      for (std::size_t i = 0; i < x.size(); ++i) {
        x[i] =
            static_cast<float>(
                static_cast<int>((i * (token_index + 7U)) % 37U) - 18) *
            0.03125F;
      }
      ...
      require(
          shared_input->upload(x, &diagnostic),
          "shared Vulkan input upload failed: " + diagnostic);

      const auto dispatch =
          resident->run_from_buffer(*context, *shared_input);
      require(dispatch.executed, dispatch.diagnostic);

      const auto* output_after = resident->output_buffer();
      require(output_after != nullptr, "resident output buffer disappeared");
      require(
          output_after->native_buffer() == output_handle,
          "resident output Vulkan buffer must be reused across tokens");

      const auto actual = output_after->download(&diagnostic);
      ...
      require(
          error <= 0.01F,
          "resident output buffer CPU parity failed");
    }

    fs::remove_all(root);
    std::cout
        << "OSM-24A resident Vulkan output: PASS\n"
        << "  device=" << context->info().device_name << "\n"
        << "  shared_input_uploads=2\n"
        << "  output_buffer_reused=PASS\n"
        << "  expert_run_no_output_readback=PASS\n"
        << "  worst_max_abs_error=" << worst_error << "\n";
    return 0;
  } catch (const std::exception& e) {
    fs::remove_all(root);
    std::cerr
        << "OSM-24A resident Vulkan output: FAIL: "
        << e.what() << "\n";
    return 1;
  }
}
